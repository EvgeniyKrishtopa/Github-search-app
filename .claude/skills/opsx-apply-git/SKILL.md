...
2. For each `isolated` group in turn, starting at the first pending one:
   ...
   - Look at the next pending group: `isolated` → continue the loop; `judgement-heavy`, or none left → **end the batch** and go to §4.8. Stopping *before* the judgement-heavy group is deliberate — it gets human review next session.
