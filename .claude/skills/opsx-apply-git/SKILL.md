...
8. Push the parent branch: `git push origin <parent-branch>`. This still goes through the normal permission gate for `git push` (it's in the `ask` list) — that gate is about tool permission, not about whether the workflow calls for pushing, so don't route around it.
