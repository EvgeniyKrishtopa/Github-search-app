...
- Running standalone or as Gate 5 — same rule either way: apply only what the user approves. Gate 5 does not silently auto-apply; that override (in `.claude/docs/git-conventions.md`) covers a task group's own commit, not harness config.
...
List what was applied, what was skipped and why, and which files changed. If this ran as Gate 5, hand control back to `opsx-apply-git` — it commits any applied changes as their own commit before the group's own commit (see its step 4).
